...
class HGCN1(tf.Module):
    def __init__(self, input_dim, output_dim, activation=tf.nn.relu):
        ...
        self.weights1 = tf.Variable(tf.random.normal([input_dim, input_dim], dtype=tf.float32), name='weights1')
        self.weights2 = tf.Variable(tf.random.normal([input_dim, output_dim], dtype=tf.float32), name='weights2')
        self.bias = tf.Variable(tf.zeros([input_dim],  dtype=tf.float32), name='bias')
        self.bias1 = tf.Variable(tf.zeros([input_dim],  dtype=tf.float32), name='bias1')
        self.bias2 = tf.Variable(tf.zeros([output_dim],  dtype=tf.float32), name='bias2')
    ...
    def __call__(self, inputs,mir,dis, wl):
        ...
        weight_emb_m = self.weights2[0:757,:] #miRNA number rows and embeding_size colums
        weight_emb_d = self.weights2[757: 757+435, :]
